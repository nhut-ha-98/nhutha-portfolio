...
export function FullProjectsSliderView({
  ...
  const scrollByPage = (direction: "next" | "prev") => {
    ...
    const delta = Math.round(element.clientWidth * 0.85) * (direction === "next" ? 1 : -1);
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-10 lg:py-14">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-bold text-[var(--ink)] sm:text-4xl">All Projects</h1>
        </div>
        <Link
          href="/"
      ...
      {!loading && error ? (
        <p className="rounded-2xl border border-red-300 bg-red-50 p-6 text-sm text-red-700">{error}</p>
      ) : null}
          ...
          {sortedProjects.map((project) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <h2 className="max-w-[90%] text-xl font-bold text-[var(--ink-soft)]">{project.name}</h2>
              ...
              {project.description.length > 0 ? (
                <ul className="mt-4 space-y-2">
                  {project.description.map((line) => (
                    <li key={line} className="text-sm leading-6 text-[var(--muted-strong)]">
                      {line}
                    </li>
                  ))}
                </ul>
              ...
              {Object.entries(project.techStack).length > 0 ? (
                <section className="mt-6 space-y-3">
                  ...
                  <div className="space-y-3">
                    {Object.entries(project.techStack).map(([group, stacks]) => (
                      <div key={group}>
                ...
                <section className="mt-6 space-y-2">
                  ...
                  <ul className="space-y-2">
                    {project.products.map((item) => (
                      <li key={item} className="text-sm leading-6 text-[var(--muted-strong)]">
                        • {item}
                      </li>
                    ))}
                  </ul>
